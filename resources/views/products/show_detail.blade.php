...
--}}
<main class="grow max-w-7xl mx-auto xl:mx-[200px] px-4 sm:px-6 lg:px-8 py-8 min-h-[75vh] bg-brand-secondary break-all sm:break-normal">
  <nav class="fade-in-effect text-sm font-medium text-brand-muted mb-6 flex flex-col sm:flex-row items-center gap-2">
    <a href="{{ route('product.catalog') }}" wire:navigate class="hover:text-brand-accent transition">Catalog</a>
    <span>›</span>
    <a href="{{ route('product.catalog') }}?filter={{ strtolower($product->category) }}" wire:navigate class="hover:text-brand-accent transition capitalize">Kategori {{ $product->category }}</a>
    <span>›</span>
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="grid grid-cols-5 gap-1 md:gap-2">
          @foreach($product->images as $img)
            <div class="w-4/5 aspect-square rounded-lg overflow-hidden border-2 cursor-pointer transition-colors border-transparent hover:border-brand-main" 
                 onclick="document.getElementById('main-product-image').src='{{ $img->display_image }}'">
              <img src="{{ $img->display_image }}" class="object-cover" alt="thumbnail">
            </div>
          @endforeach
        </div>
        @endif
      ...
      <div class="gap-y-2 flex flex-col">
        ...
        @auth
          <div class="flex gap-4 w-full">
            ...
            <button onclick="handleWhatsAppClick('{{ $product->user->whatsapp_no }}', '{{ addslashes($product->name) }}', '{{ number_format($product->price, 0, ',', '.') }}')"
...
@vite('resources/js/products/product_detail_js.js')
