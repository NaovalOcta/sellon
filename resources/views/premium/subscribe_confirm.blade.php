...
@section('title', 'Konfirmasi Pembayaran Premium Seller - SellOn')

@section('content')
<main class="grow max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 min-h-screen bg-brand-secondary">
    <div class="mb-8">
        <a href="{{ route('premium.index') }}" class="text-brand-accent hover:text-brand-main flex items-center gap-2 text-sm font-semibold transition-colors">
            <i class="fa-solid fa-arrow-left"></i> Kembali ke Premium Seller
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="space-y-6">
                ...
                <div class="p-4 bg-slate-50 rounded-2xl">
                    ...
                    <div class="w-48 h-48 bg-white border border-slate-200 rounded-xl overflow-hidden mx-auto flex items-center justify-center p-2">
                        <!-- Simple simulated QRIS code representation -->
                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=SellOnPremium15000" alt="QRIS Code" class="w-full h-full object-contain">
                    </div>
                ...
                <div class="bg-amber-50 border border-amber-100 rounded-2xl p-4">
                    ...
                    <p class="text-xs text-amber-700 leading-relaxed">
                        Silakan transfer tepat sebesar <strong>Rp{{ number_format($order->plan->price, 0, ',', '.') }}</strong>. Simpan bukti transfer Anda untuk diunggah pada form di sebelah kanan. Admin wajib memverifikasi transaksi Anda.
                    </p>
                </div>
            </div>
        </div>
        ...
        {{-- Form Konfirmasi --}}
        <div class="bg-white p-6 sm:p-8 rounded-3xl border border-slate-100 shadow-sm">
            <h2 class="text-2xl font-bold text-brand-main mb-6">Konfirmasi Pembayaran</h2>
            
            <form action="{{ route('premium.submit_payment', ['id' => $order->id]) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <input type="number" name="amount" id="amount" class="input-field w-full border @error('amount') border-red-500 bg-red-50 @else border-slate-200 @enderror px-4 py-2.5 rounded-2xl text-sm bg-slate-50 text-slate-500 cursor-not-allowed" value="{{ old('amount', (int)$order->plan->price) }}" readonly required>
                    @error('amount')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
